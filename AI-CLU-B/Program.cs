...
namespace clock_client
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            try
            {
                // Lesson: https://microsoftlearning.github.io/AI-102-AIEngineer/Instructions/10b-language-understanding-client-(preview).html
                /* Documentation on Azure.AI.Language.Converstions (can see varying api versions like 1.0.0-beta.3 for example):
                 *  https://azuresdkdocs.blob.core.windows.net/$web/dotnet/Azure.AI.Language.Conversations/1.0.0-beta.3/index.html
                */

                // CLU = Conversational Language Understanding

                // Note: had to choose subscription plan: S in order for to allow API calls
                ...
                while (userText.ToLower() != "quit")
                {
                    ...
                    if (userText!.ToLower() != "quit")
                    {
                        ...
                        // Important! Needed to use Azure.AI.Language.Conversations v = 1.0.0-beta.3
                        // in order to get access to the ConversationsProject and related classes
                        ConversationsProject conversationsProject = new ConversationsProject(lsProject, lsSlot);
                        Response<AnalyzeConversationTaskResult> response = await lsClient.AnalyzeConversationAsync(userText, conversationsProject);

                        var taskResult = response.Value as CustomConversationalTaskResult;
                        var prediction = taskResult!.Results.Prediction as ConversationPrediction;

                        Console.WriteLine(JsonConvert.SerializeObject(taskResult, Newtonsoft.Json.Formatting.Indented));
                        Console.WriteLine("--------------------\n");
                        Console.WriteLine(userText);

                        var topIntent = "";

                        if (prediction!.Intents[0].Confidence > 0.5)
                        {
                        ...
                        await Respond(topIntent, entities, prediction);
                    }
        ...
        static async Task Respond(string intent, IReadOnlyList<ConversationEntity> entities, ConversationPrediction prediction)
        {
            switch (intent)
            {
                case "GetTime":
                    ...
                    if (entities.Count > 0)
                    {
                        ...
                        foreach (ConversationEntity entity in prediction.Entities)
                        {
